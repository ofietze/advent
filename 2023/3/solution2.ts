...
const isAdjacentToGear = (engine: string[][], i: number, j: number) => {
  ...
  if (j < engine[i].length - 1 && isGear(engine[i][j + 1]))
    return `${i} ${j + 1}`; // Right
  if (i > 0 && j > 0 && isGear(engine[i - 1][j - 1]))
    return `${i - 1} ${j - 1}`; // Top Left
  if (i > 0 && j < engine[i].length - 1 && isGear(engine[i - 1][j + 1]))
    return `${i - 1} ${j + 1}`; // Top Right
  if (i < engine.length - 1 && j > 0 && isGear(engine[i + 1][j - 1]))
    return `${i + 1} ${j - 1}`; // Bottom Left
  if (
    i < engine.length - 1 &&
    j < engine[i].length - 1 &&
    isGear(engine[i + 1][j + 1])
  )
    return `${i + 1} ${j + 1}`; // Bottom Right
  return null;
};

const calculateSumOfGearRatios = (engine: string[][]): number => {
